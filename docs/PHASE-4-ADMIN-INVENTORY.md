...
| 10 | candidate-keep-as-relevant ("Keep as relevant") vs undo_scanner_decision | no-undo | Keep runs rescueCandidateSignal (a real source_signals insert plus an OpenRouter call and a full cluster visibility recompute) BEFORE recording the decision, and then sets automation_rejected_candidates.rescued_at. The undo RPC clears candidate fields only `where rescued_at is null` and never deletes the rescued signal — so Undo on a Keep revokes the allow-rule but leaves the rescued lead and the consumed spend. The insert starts private; the same persistence call then recomputes visibility under normal evidence and override rules. The lesson itself is never publication permission. Presenting Keep next to reversible controls in a redesign overstates its reversibility. | src/components/scanner/ScannerFeedbackDesk.tsx:79-87, src/app/admin/actions.ts:568-576,597-603, src/lib/automation/run.ts:1650,1920-1923, supabase/migrations/20260722170106_scanner_feedback_and_platform_pulse.sql:262-269 | high |
