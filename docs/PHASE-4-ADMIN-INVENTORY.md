...
Spot-checked roughly 60 of the highest-risk entries against source. The parity-critical write and guard locations match source: twelve actions directly pair `requireAdmin()` with `assertProductionWriteAllowed()`, signOutAdmin is deliberately unguarded, and rescueRejectedCandidate delegates to a guarded action. Column-level scanner-decision, observation-decision/Undo, visibility-override, current-patch, and bug-report-trigger claims match the migrations. Unreachability, conditional-render, and dry-run-spend claims also check out. The three discrepancies above are now corrected in the detailed entries: scan-status is documented as a write-performing GET, valid login POST has E2E coverage, and server-action/page auth coverage is no longer described as scan-route-only.
...
- **Tests:** tests/dossier.test.ts covers only the pure builder. tests/adminActionsAuth.test.ts:149-186 covers compileDossier's unauthenticated stop-before-work boundary. Its authorized read/assemble/insert/redirect and AI-fallback paths remain untested.
...
TWO SURFACES, ONE PARTITION: compileDossier lives on `/admin/compile` inside `<OperatorShell active="compile">`. The remaining operator controls live on role-aware `/scanner`; `isAdmin()` selects PublicScannerView or AdminScannerView without turning the route itself private. The current compile layout is pagehead → compile form → selected `?run=` output → newest-10 history. The scanner layout places feedback left and Latest run → Scan history → Cadence and budget in a collapsed right rail. Empty states that must survive are "No runs yet." and "No completed scan yet."; invalid dossier run ids and failed automation-settings reads still have no truthful error state.
...
SURFACE SHAPE: all four actions are reached from `/scanner` in admin mode; anonymous visitors receive PublicScannerView. The learning loop appears in this order: Action inbox → optional candidate teaching → automatic records → Wire/Asks context lanes → active lessons. Candidate and signal decisions therefore keep their rule-level Undo in a later section, while hidden observations also keep a card-level Undo beside the affected item.

ORDERING AND CAPS: the teaching query is newest-first and capped at 30 eligible rows, with two initially visible. Kept leads are the newest 20, with six initially visible. Observations are the newest 40 for the current patch, and their decision lookup is scoped to those 40 ids so a hidden row cannot lose its card Undo. Active lessons are unbounded in the admin query after revoked/expired rules are filtered, preserving an Undo for every enforced rule.
...
REVERSAL BOUNDARY: Undo revokes the feedback rule; restores a hidden observation; returns an unrescued/unexpired rejected candidate; and can indirectly restore a quarantined signal through cluster refresh. It never deletes decision/rule audit rows, reverses `rescued_at`, removes the rescue run or signal, refunds spend, or replays sources skipped while a block rule was active. Broad path/domain rules affect future intake only; stored-signal re-evaluation deliberately uses exact-URL rules.
...
- compileDossier's authorized Supabase reads, assembly, insert, redirect, AI fallback, and `use_ai` behavior remain untested; the builder and auth boundary are covered.
- On this branch, GET `/api/admin/export` still lacks route/E2E coverage despite exposing all private report columns; `tests/csv.test.ts` covers only serialization.
