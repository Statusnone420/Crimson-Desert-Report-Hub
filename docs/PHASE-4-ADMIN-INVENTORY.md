...
| 7 | moderate-report Approve / Reject / Spam | no-undo / load-bearing | The queue query selects only moderation_status='pending', so the moment a report is decided it leaves /admin and there is no control anywhere to re-decide it, un-approve it, or delete the approved_excerpts row that Approve inserts. These three buttons are also the only operator path to bug_reports.moderation_status. A redesign must keep all three plus the excerpt field together or the decision becomes unrecoverable in a different way than today. | src/app/admin/page.tsx:29-34,155-163, src/app/admin/actions.ts:111-144 | high |
...
| 10 | candidate-keep-as-relevant ("Keep as relevant") vs undo_scanner_decision | no-undo | Keep runs rescueCandidateSignal (a real source_signals insert plus an OpenRouter call and a full cluster visibility recompute) BEFORE recording the decision, and then sets automation_rejected_candidates.rescued_at. The undo RPC clears candidate fields only `where rescued_at is null` and never deletes the rescued signal — so Undo on a Keep revokes the allow-rule but leaves the published lead and the consumed spend. Presenting Keep next to reversible controls in a redesign overstates its reversibility. | src/components/scanner/ScannerFeedbackDesk.tsx:79-87, src/app/admin/actions.ts:568-576,597-603, supabase/migrations/20260722170106_scanner_feedback_and_platform_pulse.sql:262-269 | high |
...
| 12 | operator-nav EXPORT CSV | load-bearing / label mismatch | Styled as a plain <a> inside a nav of <Link> page destinations, visually identical to REPORT REVIEW / SCANNER MONITOR / COMPILE DOSSIER — but it is a one-click, no-confirmation bulk download of the entire bug_reports table including description, repro_steps, hardware_specs, driver_os and pers_id, via the service-role client. It is also the only export path in the app. A redesign must neither delete it (no other export) nor keep treating it as a nav tab. | src/components/dispatch/Chrome.tsx:154-159,186-189, src/app/api/admin/export/route.ts:6-32 | high |
| 13 | scan-dry-run-button + scan-manual-run-button | no-undo / label mismatch | Both spend real Tavily credits and real OpenRouter tokens; only persistence is skipped in dry_run, so "Test scan without publishing" is not free and not a no-op. A manual run writes across source_signals, issue_clusters, automation_rejected_candidates, patch_observations and more with no revert control. They are also the only manual scan trigger. Renaming or moving them is fine; making either easier to hit (e.g. a prominent primary action) increases irreversible spend. | src/components/ScanControls.tsx:140-155, src/app/api/admin/scan/route.ts:11-36 | high |
...
| 17 | observation Undo ("Undo — restore item and revoke rule") ↔ current-patch filter | coupled | The Context lanes desk lists only patch_observations for the CURRENT patch version, and the per-item Undo renders only on cards in that list. A hidden observation from a previous patch has no card, therefore no per-item Undo — its only remaining recovery is the rule row in Active lessons. So the Current patch override control on a different page determines whether this control exists at all. | src/lib/queries.ts:1245-1251,1265-1273, src/components/scanner/AdminScannerView.tsx:250-261 | medium |
...
| 20 | /admin/source-monitor | dead | A bare unconditional redirect() to /scanner with no requireAdmin/isAdmin call, not present in OPERATOR_NAV, and nothing in src/ links to it. Three server actions still call revalidatePath("/admin/source-monitor") for a page that renders nothing. Safe to delete, but note an anonymous visitor is bounced to the PUBLIC /scanner view rather than challenged. | src/app/admin/source-monitor/page.tsx:5-7, src/app/admin/actions.ts:442,453,466,540,607,692,726 | low |
...
| 33 | compile ?run= parameter and the 10-row Previous runs list | coupled | The output block renders only when ?run= is present; the list shows the newest 10 runs but the query by id accepts any dossier_runs uuid, so older runs remain fetchable by URL and Compile now redirects into this same parameter. A redesign that switches to client-side tab state or an inline result panel breaks both the redirect target and every bookmarked run link. | src/app/admin/compile/page.tsx:31-41,71-96, src/app/admin/actions.ts:395 | low |
...
| 2 | signOutAdmin does not just clear the cookie — it calls redirect("/admin/login") as its final statement. The inventory repeatedly describes this action as "the only effect is clearing their own cookie" and "safe in effect", and attributes the post-sign-out landing on /admin/login to requireAdmin() on a subsequent navigation. The redirect is unconditional and inside the action itself. | src/app/admin/actions.ts:42 | Where Sign out lands is a UX contract the redesign will inherit or break. It also means the unguarded endpoint does more than clear a cookie for an unauthenticated caller — it returns a redirect to the login page. |
...
| 6 | The preview/e2e harness implements only TWO Supabase RPCs — record_issue_confirmation and apply_cluster_visibility_refresh — and returns 404 {"error":"mock supabase route not found"} for everything else. record_scanner_decision, record_observation_decision, undo_scanner_decision, set_cluster_visibility_override and set_current_patch_override are all unimplemented, so in the local preview and in the Playwright suite every teaching control, both break-glass override paths, every Undo, and the current-patch override throw on submit and land on the global error boundary. | tests/e2e/mock-dev-server.mjs:914 and :919 (the only two RPCs) vs :1034 (catch-all 404) | Parity verification will be done in this harness. A redesign can render every control correctly, screenshot cleanly, pass the visual suite, and still have shipped a console where most write paths were never once exercised. It also means the e2e coverage map's admin tests only ever prove rendering, never submission. |
...
| 11 | CROSS-PAGE COUPLING: the observation moderation desk on /scanner reads patch_observations filtered to .eq("patch_version", currentPatch.version) with .limit(40). The current patch version is exactly what the "Set current patch" break-glass control on /admin writes. Setting a manual patch override therefore silently swaps the entire contents of the Context lanes desk — items under the old version become unmoderatable and unreachable, and already-rejected items from the old version can no longer be undone from the UI. | src/lib/queries.ts:1249 (filter) and :1251 (cap); written by setCurrentPatchOverride at src/app/admin/actions.ts:271 | Two controls on two different pages are wired together and neither one says so. The inventory describes the observation desk and the patch override independently. A reorganization that separates them further, or that offers the patch override more prominently, will make this invisible dependency more dangerous. |
| 12 | Hard read ceilings on the /scanner operator desks, none of them stated in the inventory: kept leads are the 20 most recent source_signals (so "Browse N older leads" only ever reaches inside a 20-row window — there is no way to teach on an older lead, and the desk's search box filters that window only); rejected candidates are capped at 30 and additionally filtered to non-expired, non-rescued and undecided; feedback rules are unbounded but filtered to non-revoked and non-expired. | src/lib/queries.ts:1152 (signals, 20), :1172 (candidates, 30), :1204-1206 (rules filter) | The inventory lists the desks and their search/show-more affordances as if they page through the full corpus. They do not. A redesign that promises "all leads" or adds filtering UI on top of these lists would be filtering a 20-row window while implying completeness. |
...
The genuine gaps are in four bands the ten partitions did not systematically cover: (1) the failure and navigation surface (the global error boundary is the only UI any admin action throw ever reaches, sign-out actually redirects, and there is no return-to after re-auth); (2) the operator tooling in package.json and tests/e2e that IS the admin workflow's preview harness — including the fact that the harness cannot exercise most admin write controls because it implements only 2 of the ~6 admin RPCs; (3) env-var-gated operator behavior with no UI (STEAM_PULSE_ENABLED, TWITCH_CLIENT_ID/SECRET, AUTOMATION_BUDGET_USD_MONTHLY) plus one real coupling break where the rescue path ignores the cap the operator saved on /scanner; and (4) hard read ceilings and cross-page coupling on the desks themselves — the flagged queue stops at 50 with no pagination while the stat cell above it shows the true count, the teachable-lead window is 20 rows, and the observation moderation desk is scoped to whatever the /admin "Current patch override" control last set.

Nothing here contradicts the covered list; it all sits alongside it. The riskiest items for a reorganization are #4 (queue caps that a redesign will assume are complete lists), #6 (a redesign verified only through the preview harness will look correct while every teach/override control is 404ing), and #11 (a control on one page silently redefining the contents of a desk on another).

---

## Inventory claims disproved (adversarial fact-check)

| # | Control | Claim | Reality | Evidence | Severity |
|---|---|---|---|---|---|
| 1 | api-admin-scan-status-get (test-coverage partition) | "writes: read-only, except cache revalidation, sweepStaleRuns() is invoked on the 404 path" — i.e. the scan-status poll is a read-only endpoint whose only write-adjacent behavior happens on the not-found branch. | The handler calls sweepStaleRuns(supabase, new Date()) unconditionally at src/app/api/admin/scan/status/route.ts:31, BEFORE the row lookup and independent of 200/404. sweepStaleRuns issues a real UPDATE on automation_runs setting status='failed', finished_at=now, errors=['stale_running_run'] for every row still 'running' older than 15 minutes (src/lib/automation/run.ts:167-184). So every poll — and there is one every 2500ms while a run is in flight (src/components/ScanControls.tsx:40,63) — performs a database write. The doc's own /scanner partition states this correctly ("a read-shaped GET performs a write"), so the two entries contradict each other; a redesign trusting "read-only" could move or re-trigger this poll and silently mass-fail live runs. | src/app/api/admin/scan/status/route.ts:31; src/lib/automation/run.ts:167-184; tests/adminScanRoute.test.ts:206-211 (the test merely happens to be the 404 case) | medium |
| 2 | admin-session-auth-primitives | "NOT PINNED: no test exercises POST /api/admin/login at all — not the 400 invalid_json branch, not the 401 invalid_credentials branch, not the deliberate 750ms timing delay ..., not the cookie attributes." | The e2e suite does exercise POST /api/admin/login and asserts on it: signInAsAdmin() posts to /api/admin/login with the admin password and asserts expect(response.ok()).toBe(true) (tests/e2e/public-visual.spec.ts:66-69). That helper is the precondition for every operator-surface test, and the subsequent authenticated navigations prove the cookie it sets is accepted by isAdmin()/requireAdmin(). Only the 400/401/timing branches are genuinely unpinned. The same inventory contradicts itself in entry admin-login-post, which correctly notes the route is "[a]lso hit directly by e2e (tests/e2e/public-visual.spec.ts:67)". | tests/e2e/public-visual.spec.ts:66-69; src/app/api/admin/login/route.ts:5-27 | medium |
| 3 | api-admin-scan-post (test-coverage partition) | "This is the ONLY admin surface whose auth guard is genuinely asserted by a test." | The /admin page's requireAdmin() redirect is also asserted by a test: after clicking Sign out the e2e expects /admin/login, then does page.goto("/admin") and expects to land on /admin/login again (tests/e2e/public-visual.spec.ts:1053-1056). That is a real assertion of the requireAdmin guard at src/app/admin/page.tsx:25, on a different surface than POST /api/admin/scan. The inventory's own e2e-admin-auth-routing entry describes exactly this test, so the "ONLY" claim is contradicted within the same document. | tests/e2e/public-visual.spec.ts:1053-1056; src/app/admin/page.tsx:25; src/lib/adminGuard.ts:20-22 | low |

**Fact-check assessment:**

Spot-checked roughly 60 of the highest-risk entries against source. The parity-critical columns (writes and guards) for the real operator controls are accurate to a level I could not break: every `requireAdmin()`/`assertProductionWriteAllowed()` pair in src/app/admin/actions.ts is where the doc says it is (112-113, 168-169, 198-199, 235-236, 264-265, 282-283, 399-400, 447-448, 458-459, 471-472, 620-621, 709-710), signOutAdmin (39-43) and rescueRejectedCandidate (697-705) genuinely lack their own guards, and the "12 guarded actions" count is exactly right. Column-level write claims check out against the SQL: record_scanner_decision (20260722170106:153-227), record_observation_decision + the recreated undo (20260724200000:93-212), set_cluster_visibility_override (20260722170106:444-486, including the visibility_restore_* snapshot/restore semantics), set_current_patch_override (20260710021010:24-55), and the bug_reports trigger (20260710001212:97-160). Unreachability claims are all true (runRedditMonitor, setAutomationPaused, RejectedArchive/rescueRejectedCandidate, DELETE /api/admin/login, the expires_at form field, no middleware.ts). Conditional-render claims in AdminScannerView (169/171, 250-262, 663-667) and ScannerFeedbackDesk (78-88 outside the learning gate, 89/126-131) are exact, as is the dry_run skip set (run.ts:2373/2391/2401/2424/2463/2502) and the fact that dry_run still buys 5 Tavily queries (budget.ts:213-231). The three errors I could disprove all live in the test-coverage partition; the most consequential is the scan-status entry labeling a write-performing GET as read-only, which the doc's own scanner partition contradicts.
...
- **Does:** Reads all issue_clusters (id, title, fix_status, admin_override, lifecycle_reason, admin_visibility_override, admin_visibility_reason, admin_visibility_changed_at, is_public) ordered by title.
...
- **Does:** Chooses which issue cluster the report is filed under; options are every cluster from readAdminClusters, ordered by title, defaulting to the report's existing cluster_id.
- **Backing:** src/app/admin/page.tsx:141-148; consumed at src/app/admin/actions.ts:116 and :135
- **Inputs:** Option values are cluster uuids; "" = No cluster. No search, no grouping, no cap. defaultValue = report.cluster_id ?? ""
...
- **Quirks:** Coupled to ALL THREE buttons, not just Approve: moderateReport writes cluster_id on rejected and spam decisions too (actions.ts:135). Pressing Reject while the select shows "No cluster" strips an existing cluster link. There is no visible label, and the list is unbounded — with many clusters this becomes an unusable dropdown farm, which is exactly the pattern the visibility section escaped from.
...
- **Inputs:** clusters prop = every cluster WITHOUT an override; internal fields cluster_id (hidden), visibility (select, default force_hidden), reason (textarea, minLength 3 / maxLength 500, required), confirm_override (checkbox value="true", required)
...
- **Quirks:** 1) The approved_excerpts INSERT is the irreversible part: no delete/edit control for excerpts exists anywhere in the app (only reads at src/lib/queries.ts:463, :881, src/app/api/reports/route.ts:89, src/lib/automation/run.ts:1289). 2) Published excerpts are NOT filtered by moderation_status (src/lib/queries.ts:433-475), so an excerpt stays public even after the report is later rejected or marked spam. 3) approved_excerpts has no unique constraint on report_id (schema.sql:48-53), so approving the same report twice with text inserts duplicate public quotes; verified_report_count dedupes by report_id so counts do not move. 4) Ordering is deliberate and load-bearing: excerpt insert must precede the visibility refresh (comment at actions.ts:153-156). 5) Old and new clusters are refreshed in Set insertion order — old first, then new (test at :245-260 asserts the order). 6) The queue only lists moderation_status='pending' (src/app/admin/page.tsx:29-34, limit 50, oldest first), so once decided a report has no UI route back; combined with 'pending' being absent from DECISIONS, moderation is one-way from the operator's chair.
...
- **Inputs:** name="cluster_id", defaultValue = report.cluster_id ?? "". Options: "" ("No cluster") plus every cluster in the table, ordered by title.
...
- **Quirks:** Unbounded list — every cluster in the database renders as an <option> on every flagged report (no search, no grouping, no category hint); this is the same data the VisibilityOverrideBrowser deliberately hides behind a search box. Also note that changing cluster on an ALREADY-approved report is what triggers the two-cluster refresh path, but that path is unreachable from this page because only pending reports are listed.
...
- **Tests:** —
- **Quirks:** It is a nested <details> inside the page's own <details> (page.tsx:227) — two levels of disclosure before any control is reachable. The entire cluster list is shipped to the client as props on every /admin render, even when the disclosure is never opened.
...
- **Does:** Reads six data sets, builds one deterministic Markdown dossier, optionally has a free OpenRouter model rewrite the prose, inserts the result as a new dossier_runs row, then redirects to that run.
...
- **Writes:** dossier_runs.markdown; dossier_runs.provider; dossier_runs.stats (jsonb: totalSignals, totalDirectReports, totalVerifiedReports, pendingCount); dossier_runs.id / dossier_runs.created_at via table defaults; READS ONLY: bug_reports (approved: category, platform, cluster_id, evidence_url, repro_steps, issue_title), bug_reports pending head-count, issue_clusters (id, title, fix_status, confidence — ALL rows, unfiltered), source_signals where public_status='public' (cluster_id, source, source_url, title, summary, category, observed_at), approved_excerpts + joined bug_reports (newest 1000), official_patch_notes via getCurrentPatchMetadata
...
- **On failure:** Any of the five Supabase reads failing throws `<label> read failed: <message>` (throwReadError, :83-85) BEFORE any write, so nothing is persisted — labels are "approved reports", "issue clusters", "community signals", "verified reports", "pending reports". getCurrentPatchMetadata never throws: it silently degrades to the hardcoded fallback patch (src/lib/officialPatch.server.ts:79-93), so a broken patch table produces a dossier titled with the wrong patch instead of an error. AI drafting failure is invisible: draftDossierWithAi returns null on non-2xx, non-free cost, short content, or any throw, and the action keeps the deterministic markdown with provider="deterministic". The final insert failing throws error.message and writes nothing.
- **Tests:** tests/dossier.test.ts (covers buildDeterministicDossier only — the pure builder); NO test exercises compileDossier itself: no read/assemble/insert/redirect coverage, no coverage of the AI fallback path
- **Quirks:** Not idempotent — every submit appends another dossier_runs row; double-clicking produces duplicate runs with no dedupe, no upsert, no run key. Partial failure leaves nothing behind (single write, last), so the real risk is the opposite: silent degradation. src/lib/ai.ts:40 issues the OpenRouter fetch with NO timeout/AbortController and the compile page exports no maxDuration, so a hung provider hangs the whole action to the platform limit. `.single()` result is used as run.id at :395 with no null check. approved_excerpts is capped at the newest 1000 rows (:313) — verifiedReportCount silently under-counts past that. issue_clusters is read unfiltered (no patch, no visibility filter), so force_hidden clusters still feed counts; only clusters with evidence>0 reach the Top issues table (src/lib/dossier.ts:60). Only public_status='public' signals are counted, so the dossier's signal totals will not match any admin-side lead count. dossier_runs.provider is load-bearing UI: the page prints "DETERMINISTIC" vs "AI DRAFT · <PROVIDER>" from it (compile/page.tsx:21-23).
...
- **Does:** Opts this one compile into an AI prose rewrite of the deterministic dossier; aggregates stay deterministic either way.
...
- **Writes:** dossier_runs.markdown (AI prose instead of deterministic text); dossier_runs.provider ('openrouter' instead of 'deterministic')
...
- **Tests:** —
- **Quirks:** An invalid ?run= value is indistinguishable from "no run selected" — no not-found state. The runs list is hard-capped at 10 with no pagination, so older dossiers become unreachable through the UI even though the rows persist.
...
**Surface notes.** SCOPE: lines 281-469 of D:\CD Report Hub\src\app\admin\actions.ts plus the two UI surfaces that reach them. Helper functions at :79-109 serve compileDossier exclusively — relatedReport (:79) unwraps PostgREST's join value, which arrives as either an object or a one-element array; throwReadError (:83) is the uniform read-failure thrower; distinctVerifiedReports (:87) and distinctVerifiedClusterRows (:102) both de-duplicate approved_excerpts by report_id keeping the NEWEST excerpt (rows arrive created_at desc), so one report contributes at most one verified entry. They are not controls, but they are the reason the dossier's verified counts differ from a raw excerpt count.  TWO DIFFERENT SURFACES, ONE PARTITION. compileDossier lives on /admin/compile inside <OperatorShell active="compile">. Everything else operator-facing here lives on /scanner, which is role-aware: src/app/scanner/page.tsx:21 uses the non-throwing isAdmin() so anonymous visitors get PublicScannerView and admins get AdminScannerView. A redesign that consolidates "the admin area" must not assume /admin is the only admin surface — the scanner's spend controls are on a public route.  LAYOUT / GROUPING as it exists today. /admin/compile: pagehead, then a single .compile-band form (checkbox, submit, note), then the output block only when ?run= resolves, then a "Previous runs" list of 10. /scanner admin: a two-column .operator-workbench with the feedback desk on the left and an <aside aria-label="Latest run and scanner settings"> on the right containing, in order, the Latest run block, a "Scan history and diagnostics" <details>, and the "Scanner cadence and budget" <details>. That ordering buries the highest-stakes controls last and collapsed.  EMPTY STATES that exist and must survive: "No runs yet." (compile/page.tsx:96) and "No completed scan yet." (AdminScannerView.tsx:572). There is NO empty/error state for an invalid ?run= id, and no error state anywhere for a failed automation_settings read on this form — src/lib/queries.ts:899-902 degrades a failed read to {paused:false, updatedAt:null} and only console.errors, so the form can render "not paused" while the truth is unknown.  DEAD OR ORPHANED, three items, all easy to delete silently: runRedditMonitor (permanently disabled by the hardcoded `reddit: false` at src/lib/env.ts:35, no UI caller, one test pinning it dead); the exported setAutomationPaused action (no UI caller, two tests pinning it); and the modelPreset hidden input (single allowed value equal to the default). All three still have live tests or constraints attached.  REVALIDATION ASYMMETRY worth resolving: setAutomationPaused and setScannerPolicy each revalidate /admin, /scanner, /admin/source-monitor and all public surfaces; runRedditMonitor revalidates /scanner, /admin/source-monitor and public surfaces; compileDossier revalidates NOTHING and relies on redirect plus force-dynamic. /admin/source-monitor is a real page (src/app/admin/source-monitor/page.tsx), so that target is not stale today.  GUARD PATTERN is uniform and must be preserved verbatim on any new action: `await requireAdmin()` (redirects to /admin/login, does not throw) followed by `assertProductionWriteAllowed()` (throws "preview writes disabled" when VERCEL_ENV==='preview'). The library layer beneath — src/lib/automation/settings.ts — has no guard of its own and accepts an injected client, so it is only as safe as its callers.  BLAST RADIUS SUMMARY for the automation switches, the thing most likely to be underestimated: a single jsonb row (automation_settings where key='scanner', schema at supabase/migrations/20260705201242_automation_settings.sql, seeded with {"paused": false}) is read by the hourly cron (src/app/api/cron/keepalive/route.ts:32), the manual scan route (src/app/api/admin/scan/route.ts:25), the run engine's budget (src/lib/automation/run.ts:2482, src/lib/automation/budget.ts:188-237), the admin dashboard and scanner desk (src/lib/queries.ts:899, :1212), the PUBLIC scanner scoreboard (src/lib/queries.ts:1596-1606, "scannerActive") and the PUBLIC Patch Radar (src/lib/radar.server.ts:535, 566-567, publishing paused + cadenceMinutes). Both writers replace the whole blob with no merge and no locking. Pausing is publicly visible; pausing does NOT stop manual scans; and supplying a policy at all switches the budget engine off the AUTOMATION_BUDGET_USD_MONTHLY env path (the 'budget_capped' skip reason becomes unreachable, budget.ts:189/207).  COVERAGE GAP: compileDossier has no test of any kind — the reads, the AI fallback, the dossier_runs insert and the redirect are all unpinned. tests/dossier.test.ts covers only the pure builder. If the redesign touches compile, there is no safety net.
...
- **Does:** Records one durable operator decision about a discovered page: writes an immutable decision row plus one active feedback rule that teaches future scanner runs to allow or block that URL/path/domain; for target_kind=signal it also quarantines the already-kept public lead.
...
- **Does:** Declares an auto-rejected candidate actually relevant: runs the full rescue pipeline (LLM extraction + signal persistence + cluster recompute), records an 'allow' rule for the exact URL, and marks the candidate rescued so it leaves the desk.
...
- **Writes:** scanner_decisions (insert, decision='relevant'); scanner_feedback_rules (insert, action='allow', scope_type='exact_url'); scanner_feedback_rules.revoked_at/superseded_by_rule_id on prior same-URL rules; automation_rejected_candidates.decision_id/.feedback_rule_id/.decided_at; automation_rejected_candidates.rescued_at (actions.ts:598-602); plus everything rescueCandidateSignal writes: automation_runs (insert 'running' + finalize), source_signals (insert or seen_count update), issue_clusters visibility/stat columns
...
- **Tests:** tests/adminActions.test.ts:645-662; tests/e2e/public-visual.spec.ts:957 asserts the button is visible
- **Quirks:** UNDO DOES NOT REACH THIS. undoScannerDecision revokes the allow rule and marks the decision undone, but it only clears decision_id/feedback_rule_id/decided_at 'where rescued_at is null' (migration 20260724200000_observation_moderation.sql:188-193) — the rescued candidate keeps its decision_id, stays filtered out of the desk (src/lib/queries.ts:1169-1170), and the created source_signals row + automation_runs row are never removed. This is the one control in this partition that spends money (an OpenRouter LLM call) on click. The canned reason is stored verbatim into scanner_decisions.reason and scanner_feedback_rules.reason and shown in the Active lessons ledger — every keep looks identical there.
...
- **Tests:** —
- **Quirks:** NO action-level test exists — tests/adminScannerView.test.ts:12 only mocks it, and tests/scannerFeedbackMigration.test.ts covers undo_scanner_decision, not record_observation_decision. That is the biggest verification gap in this partition. The 'teach' half is weaker than it reads: a blocked exact URL stops future intake at run.ts:1051-1075 (before the observation reroute at run.ts:996-1008), but community_ask items are deduped by an ask-SERIES hash rather than URL (src/lib/automation/observations.ts:54-61), so a serialized campaign posting a NEW url each day is not stopped by an exact_url rule — only a domain rule would catch it. Also note it never calls refreshClusterVisibility because observations carry no cluster_id by design (observations.ts:16-18).
...
- **Writes:** identical to the Keep-as-relevant path: scanner_decisions, scanner_feedback_rules (action='allow'), automation_rejected_candidates.decision_id/.feedback_rule_id/.decided_at/.rescued_at, plus rescueCandidateSignal's automation_runs / source_signals / issue_clusters writes
...
- **Does:** Marks one decision undone, revokes every rule that decision created, returns an unrescued candidate to the teaching desk, restores a hidden observation to public, and bumps the affected cluster so a signal quarantine can be recomputed away.
...
- **Quirks:** ASYMMETRIC REVERSIBILITY — the single most important fact for a redesign. Undo makes the FUTURE-facing rule inert and restores row state (observation is_public, candidate desk membership, signal visibility via recompute), but it never deletes the scanner_decisions/scanner_feedback_rules audit rows, never restores a rescued candidate (rescued_at is not cleared and blocks the reset), never removes the source_signals row or automation_runs row a Relevant rescue created, and cannot replay sources that were dropped by the rule while it was active — those runs already wrote automation_rejected_candidates rows with reason=<decision> and are gone. Undo for a signal quarantine is also conditional on the signal still having a cluster_id. There is no confirmation step and no 'redo'.
...
- **Does:** Revokes that lesson and marks its decision undone, removing the rule from the ledger and from future scanner matching.
...
- **Does:** Before the decision is written, spends an OpenRouter LLM call to extract a structured signal from the candidate, opens and finalizes an automation run ledger row, upserts a source_signals row, and recomputes the resolved cluster — deliberately SKIPPING the pre-screen and keep gates because the operator already judged it relevant.
...
- **Inputs:** No operator inputs — derived entirely from the candidate row (title, url, source_domain, source_published_at, snippet) read at actions.ts:545-552.
...
- **Tests:** tests/adminActions.test.ts:645-662 (rescue precedes the decision write); tests/adminActions.test.ts:708-721 (failed rescue writes nothing); tests/automationRun.test.ts:2615,2658,2717,3498 (candidatesRescued accounting)
- **Quirks:** A click on a small text button issues a paid model call and creates a run row that appears in 'Scan history and diagnostics' and in public scanner counters. Nothing in the button copy says so. This is the only irreversible write in the whole learning loop: Undo cannot delete the created signal or run, and the rescued candidate never returns to the desk.
...
**Surface notes.** SURFACE SHAPE. All four actions are reached from exactly one page: /scanner rendered in admin mode (src/app/scanner/page.tsx:20-47 — isAdmin() picks AdminScannerView, anonymous visitors get PublicScannerView; there is no /admin route for any of this). Within AdminScannerView the learning loop is spread across four sections in this order: (1) "Action inbox" summary strip with the counts "<n> optional teaching candidates" and "<n> active scanner lessons" (:519-535); (2) "Teach the scanner · Optional" workbench holding the candidate desk (:538-554) with the settings rail beside it; (3) "Automatic records — What the scanner kept" holding per-lead Remove-bad-lead (:620-638, six recent leads plus a "Browse N older leads" disclosure); (4) "Context lanes — Wire and Asks on the Brief" holding observation reject/undo (:640-656); (5) "Active lessons — What the scanner will remember" holding every Undo (:658-668). The undo for a candidate/signal decision is therefore physically separated from the card that created it — that separation is the current design, and any reorganization must keep some path from a decision to its Undo.  ORDERING AND CAPS THAT ARE LOAD-BEARING. Teaching desk: newest-first, capped at 30 rows server-side (unexpired, unrescued, undecided — src/lib/queries.ts:1162-1172), and only the first TWO render without a click (DEFAULT_VISIBLE_CANDIDATES, ScannerFeedbackDesk.tsx:10,191). Kept leads: newest observed_at first, 20 rows total (queries.ts:1146-1152), split 6 visible / rest in a disclosure (AdminScannerView.tsx:354-355). Observations: current patch only, observed_at desc, 40 rows (queries.ts:1246-1251), with their active-decision lookup scoped to exactly those 40 ids so a hidden item cannot lose its Undo (:1265-1273). Active lessons: newest first, revoked and expired rules filtered out twice (:1201-1206, :1305-1307).  EMPTY STATES (four distinct strings, all easy to lose): "Nothing needs teaching right now. New auto-rejects remain private and expire on their own." (ScannerFeedbackDesk.tsx:173); "No kept source leads yet." (AdminScannerView.tsx:628); "No observations recorded for this patch yet." (:654); "No scanner lessons yet. Decisions you record above will appear here with an Undo control." (ScannerFeedbackDesk.tsx:230). Plus three pre-migration fallback sentences and one Steam-review explanation sentence.  REVERSIBLE ROW STATE vs IRREVERSIBLE TRAINING SIGNAL — the summary the redesign needs: - Reversible by Undo: scanner_feedback_rules.revoked_at (rule stops matching), patch_observations.is_public (restored to true), automation_rejected_candidates.decision_id/feedback_rule_id/decided_at (candidate returns to the desk, only while rescued_at is null and expires_at has not passed), and source_signals.public_status for a quarantined signal — but only indirectly, via the visibility_revision bump plus refreshClusterVisibility, and only when the signal still has a cluster_id. - Never reversed: scanner_decisions rows (append-only audit; undone_at is a tombstone, not a delete), scanner_feedback_rules rows themselves (revoked, never deleted, and their superseded_by_rule_id chain is permanent), automation_rejected_candidates.rescued_at, and everything a Relevant rescue created (automation_runs ledger row, source_signals row, cluster stat changes, real money spent on the LLM call). - Irreversible in practice even though no row says so: every source the scanner skipped while a block rule was active. Blocked candidates are recorded as automation_rejected_candidates with reason=<decision> and feedback_rule_id and counted into automation_runs.operator_rules_matched (run.ts:1058-1075); revoking the rule later does not replay them. A broad source_path/source_domain rule is therefore the highest-blast-radius control in this partition, which is why it costs an extra confirmation. - One asymmetry worth naming: broad (path/domain) rules gate FUTURE intake only; when re-evaluating already-stored signals the engine deliberately keeps only exact_url rules (run.ts:1736-1739), so a domain lesson never retroactively rewrites existing evidence counts.  VERIFICATION GAPS FOUND. rejectObservationAndTeach has no action-level test anywhere (tests/adminScannerView.test.ts:12 mocks it; tests/scannerFeedbackMigration.test.ts covers undo only) — the branchiest new action in the partition is pinned by render tests alone. The expires_at parameter of recordScannerDecision is fully implemented server-side and in SQL but no UI emits it. src/components/scanner/RejectedArchive.tsx (Rescue button, archive search, "Show N more before they expire →" disclosure) is not imported by any file in src and is currently dead UI.
...
- **Tests:** tests/adminScannerView.test.ts:103 "keeps every retained lead reachable and gives each one an explicit teaching action"
...
- **Tests:** tests/adminScannerView.test.ts:103 "keeps every retained lead reachable"
- **Quirks:** Hard-coded split at index 6 (:354-355) against a query capped at 20 (src/lib/queries.ts:1152), so this can hold at most 14 leads and there is no pagination beyond 20 — leads 21+ are unreachable in the admin UI entirely. The test name 'keeps every retained lead reachable' pins the requirement that nothing is hidden without a disclosure, so a redesign that truncates the list without an expander will fail it. Singular/plural handled inline at :632.
...
- **Does:** Records a permanent allow-rule for this exact URL, re-runs the candidate through LLM extraction to persist it as a real source signal, and marks the candidate rescued.
...
- **Writes:** automation_runs (insert via createRunLedger, then update at finalize — src/lib/automation/run.ts:2666 and the finally at :2744); source_signals (insert or reobserve upsert via persistOneSignal/upsertSignal — src/lib/automation/run.ts:1913); issue_clusters (stats + possible promotion via refreshClusterStats — src/lib/automation/run.ts:1922); scanner_decisions.id/.candidate_id/.signal_id(null)/.target_url/.target_url_hash/.source_domain/.decision('relevant')/.reason (migration 20260722170106:153-171); scanner_feedback_rules.id/.decision_id/.action('allow')/.decision('relevant')/.scope_type('exact_url')/.scope_value/.reason/.confirmed_at(now)/.expires_at(null) (migration 20260722170106:173-193); scanner_feedback_rules.revoked_at + .superseded_by_rule_id on any prior active rule with the same scope_type+scope_value (migration 20260722170106:195-201); automation_rejected_candidates.decision_id/.feedback_rule_id/.decided_at (migration 20260722170106:203-209); automation_rejected_candidates.rescued_at (src/app/admin/actions.ts:598-602); external: one OpenRouter LLM call, capped at MAX_RESCUE_LLM_CALLS = 1 (src/lib/automation/run.ts:289, applied :2664)
...
- **Quirks:** THREE things a redesign will get wrong here. (a) This button spends money and creates a scan-history row. It fires a real OpenRouter LLM call and inserts an automation_runs ledger entry with intent "rescue_candidate" (run.ts:2666, :2680). Nothing in the UI hints at cost; the label reads like a local bookmark. (b) It is NOT symmetric with Undo. undo_scanner_decision only returns candidates to the desk `where decision_id = p_decision_id and rescued_at is null` (migration 20260722170106:264-269), and this path sets rescued_at at actions.ts:600. So undoing a Keep-as-relevant revokes the allow rule but the candidate does NOT come back to the desk and the created source_signal is NOT removed. "Undo" is a partial undo only for this decision. (c) Legacy-database leniency is asymmetric: if the record_scanner_decision RPC is missing, a relevant decision silently continues without writing any decision or rule row (legacyRelevantRescue, actions.ts:591-595) — the signal is rescued and no Undo control ever appears for it. A rejection in the same situation throws. Also note the allow rule supersedes any prior block rule on the same exact URL (migration :195-201), which is the intended mechanism for correcting the scanner but is invisible in this UI.
...
- **Does:** Records a permanent allow-rule for the candidate's exact URL, re-extracts it through the LLM into a real source signal, and marks it rescued — identical to the teaching desk's Keep as relevant.
...
- **Writes:** automation_runs (insert + finalize — src/lib/automation/run.ts:2666, :2744); source_signals (upsert via persistOneSignal — src/lib/automation/run.ts:1913); issue_clusters (stats/promotion via refreshClusterStats — src/lib/automation/run.ts:1922); scanner_decisions (full row, decision='relevant' — migration 20260722170106:153-171); scanner_feedback_rules (full row, action='allow', scope_type='exact_url' — migration 20260722170106:173-193); scanner_feedback_rules.revoked_at/.superseded_by_rule_id on prior same-scope rules (migration 20260722170106:195-201); automation_rejected_candidates.decision_id/.feedback_rule_id/.decided_at (migration 20260722170106:203-209); automation_rejected_candidates.rescued_at (src/app/admin/actions.ts:598-602); external: one OpenRouter LLM call (MAX_RESCUE_LLM_CALLS = 1, src/lib/automation/run.ts:289)
...
- **Inputs:** clusters.length, where clusters = autoRows from src/app/admin/page.tsx:47 = every issue_cluster with a falsy admin_visibility_override, read unbounded and ordered by title (src/lib/adminClusters.ts:19-24).
- **Writes:** read-only
- **Guard:** N/A
- **Revalidates:** —
- **On failure:** If readAdminClusters hits a missing-column error it falls back to a legacy select (src/lib/adminClusters.ts:37-47) that still returns id/title/admin_visibility_override/is_public, so this count stays correct; only admin_visibility_reason and admin_visibility_changed_at degrade to null (which affects the forced-cluster cards above, not this browser).
- **Tests:** —
- **Quirks:** The word "records" here is doing vocabulary work — per the project's locked number-vocabulary this counts engine-owned issue clusters, not published issues. The number deliberately excludes forced clusters (they live in the list above), so it DROPS by one every time an override is applied and rises when one is reset. An operator watching this number will see it move for reasons unrelated to the scanner. Also: the whole cluster list is shipped to the client as props on every /admin render, so this is an unbounded client payload that grows with the issue count.
...
- **Tests:** tests/e2e/public-visual.spec.ts:1015 asserts the "Previous runs" label is visible
...
- **Does:** When checked, asks the compile action to send the deterministic dossier to OpenRouter for a prose rewrite; unchecked (or disabled) compiles deterministically.
...
- **Writes:** Indirectly dossier_runs.provider and dossier_runs.markdown — see compile-now-button
...
- **Revalidates:** —
- **On failure:** If AI drafting is requested but fails, draftDossierWithAi returns null (src/lib/ai.ts:38-64 — every non-OK response, every non-free-cost response, every network throw just `continue`s) and the action keeps the deterministic markdown with provider "deterministic" (actions.ts:373-378). Silent fallback: the run is still saved, the mode line just says DETERMINISTIC.
- **Tests:** —
- **Quirks:** Named `use_ai` and compared against the literal "on" — if a redesign gives the input an explicit `value`, the server comparison breaks silently and AI drafting stops working with no error. The checkbox is coupled to the compile-ai-availability-probe for its disabled state and to the op-note beside the button for its meaning; splitting them loses the "AI only rewrites prose" promise. The label wraps the input (`.report-check`, globals.css:2121) rather than using htmlFor.
...
- **Does:** NOT A PAGE. Triggers a file download of every bug report as CSV: the response carries content-disposition: attachment; filename="cd-reports-YYYY-MM-DD.csv".
...
- **Inputs:** None. No date range, no status filter, no column picker — it always exports all 22 columns for all bug_reports, newest first. (The route itself belongs to the API-routes partition; listed here because it is a NAV control.)
- **Writes:** read-only — but it EXFILTRATES the full bug_reports table including description, repro_steps, hardware_specs, driver_os and pers_id
...
**Surface notes.** SCOPE: I inventoried src/app/admin/compile/page.tsx, src/app/admin/login/page.tsx + LoginForm.tsx, src/app/admin/source-monitor/page.tsx, and the OperatorShell/OPERATOR_NAV half of src/components/dispatch/Chrome.tsx (lines 154-219). I read the handlers they call: compileDossier and signOutAdmin (src/app/admin/actions.ts:39-43, 281-396), requireAdmin (src/lib/adminGuard.ts), features() (src/lib/env.ts), assertProductionWriteAllowed (src/lib/previewGuard.ts), draftDossierWithAi (src/lib/ai.ts:35-65), DossierOutput/CopyDossierButton (src/components/DossierOutput.tsx), and the dossier_runs schema (supabase/migrations/20260705192906_schema.sql:72-78). I did NOT inventory /api/admin/login or /api/admin/export as routes (other partitions) — I read them only far enough to state what the operator sees.  LAYOUT / GROUPING, /admin/compile: three horizontal rules stack the page top to bottom — pagehead (title + dek, paddingBottom 32 inline), then .compile-band (border-top 2px --rule-strong, a wrap-enabled flex row holding checkbox + button + note, globals.css:2718), then the optional .compile-output block (1px rule, globals.css:2722), then .compile-runs (1px rule, globals.css:2719). ORDER IS SEMANTIC: the newest run appears between the compile controls and the history, so the page reads "compile → here is what you just made → here is what you made before". Font sizes are inline overrides (fontSize 44 on the h1 at page.tsx:49), not classes.  EMPTY STATES: /admin/compile has exactly one — "No runs yet." (page.tsx:96), which as noted also covers a failed query. There is NO empty state for the output block: with no ?run= the entire block is absent rather than showing a placeholder, so a first-time operator sees a form and a history list and nothing that explains what a dossier looks like.  TWO SIGN-IN FORMS EXIST IN THIS CODEBASE. The full-page one at /admin/login (LoginForm.tsx, in my partition) and the footer popover (src/components/AdminControls.tsx, public chrome). They post to the same endpoint and show the same "Wrong password." text, but they differ: the popover sets autoComplete="current-password", clears the password on success, and has a Cancel button; the full-page form has none of those. The popover is the one covered by e2e (public-visual.spec.ts:1020-1042); the full-page form has NO test at all — e2e authenticates by POSTing the API directly (spec:66-69). Parity risk: a redesign could delete /admin/login believing the popover covers it, and every direct link/redirect to /admin/login (adminGuard.ts:21, actions.ts:42) would 404.  UNTESTED CONTROLS (parity risk ranked): the login form end-to-end, Sign out, EXPORT CSV, the copy-to-clipboard button, the run-history links, the ?run= load path, and the /admin/source-monitor redirect. The only automated coverage in this partition is the compile page's static chrome (heading, button, note, "Previous runs") plus the admin-compile.png screenshot, and the three revalidatePath("/admin/source-monitor") assertions in tests/adminActions.test.ts.  NO /admin LAYOUT EXISTS. src/app/admin contains only actions.ts and the four page files — no layout.tsx, no loading.tsx, no error.tsx. Consequences a redesign inherits: (1) every operator page must import OperatorShell itself, and each must pass its own `active` key; (2) there is no shared operator error boundary, so any server-action throw falls through to the public-chromed src/app/error.tsx and the operator loses the amber chrome mid-task; (3) there is no route-group auth — each page repeats requireAdmin(), and there is no middleware.ts anywhere in the repo to catch a page that forgets.  CHROME IS THE AUTH TELL. Crimson topline = signed out (PublicShell), amber topline + "Operator console · signed in" = signed in (Chrome.tsx:173-176). /admin/login is deliberately crimson (comment at src/app/admin/login/page.tsx:4-5). That colour contract is the only persistent signal of session state — there is no session-expiry countdown and no expiry warning anywhere.  CONTROL DENSITY: the operator nav is 6 items wide (4 nav + Sign out + the wordmark above it) and 4 of the 6 look identical while doing three different things (navigate, download, submit a form). That is the single biggest thing to preserve deliberately rather than by accident.
...
- **Does:** Downloads every row of the bug_reports table as a CSV file. This is the only bulk data-exfiltration control on the surface.
- **Backing:** src/app/api/admin/export/route.ts:31-48
- **Inputs:** none. GET with no query params: no date range, no moderation_status filter, no platform/patch filter, no pagination, no column selection, no row cap. There is no way to export a subset.
...
- **Writes:** automation_runs (INSERT: started_at, status='running', mode, budget_monthly_usd, budget_remaining_before_usd, skips, progress — src/lib/automation/run.ts:2030-2051; UPDATE on finish at run.ts:2096-2103); automation_settings (READ only, via getAutomationControlState — src/lib/automation/settings.ts:129-133); then, in the background run: source_signals, issue_clusters, automation_rejected_candidates, signal_observation_events, steam_pulse_snapshots, steam_review_receipts, platform_context_snapshots, scanner_feedback_rules (table list from grep of src/lib/automation/run.ts); mode=manual additionally runs syncOfficialPatchNote (run.ts:2502-2508), which dry_run skips; external spend: Tavily search credits and OpenRouter LLM tokens — real money, not reversible
...
- **Does:** Renders the dossier compile form and, when ?run is present, a previously generated dossier.
...
- **Revalidates:** —
- **On failure:** No session -> redirect. A malformed ?run makes .single() error; the destructure at :39-40 ignores `error`, so `current` stays null and the page silently renders without the dossier rather than reporting anything.
- **Tests:** tests/dossier.test.ts
- **Quirks:** Guard runs BEFORE `await searchParams` (:26 then :27) — correct ordering that a refactor could easily invert. The silent-null failure on a bad ?run means a broken deep link looks identical to 'no run selected'.
...
- **Does:** Streams every bug_reports row (23 fixed columns) as a CSV attachment.
- **Backing:** src/app/api/admin/export/route.ts:31-48 (guard at :32; COLUMNS at :6-29)
- **Inputs:** None — no filters, no date range, no pagination.
- **Writes:** read-only
...
- **Quirks:** The one operator nav item that leaves the app entirely (full navigation, no client routing) — hence the deliberate <a>/<Link> split at Chrome.tsx:186-199. It exports the full table including submitter-adjacent fields (pers_id, hardware_specs, driver_os) with no row cap. An expired session turns this nav item into a JSON 401 page with no way back except the browser Back button.
...
- **Does:** Reads every issue_clusters row (all of them, no filter, no limit) ordered by title, with a legacy-column fallback.
- **Backing:** src/lib/adminClusters.ts:16
- **Inputs:** Service-role Supabase client. Primary select: id, title, fix_status, admin_override, lifecycle_reason, admin_visibility_override, admin_visibility_reason, admin_visibility_changed_at, is_public. Ordered by title.
- **Writes:** read-only
...
- **Does:** Dumps every bug_reports row, newest first, as a CSV attachment named cd-reports-YYYY-MM-DD.csv.
- **Backing:** src/app/api/admin/export/route.ts:31
- **Inputs:** No parameters. Fixed 22-column list (route.ts:6-29): id, created_at, patch_version, platform, category, severity, frequency, issue_title, description, repro_steps, expected_behavior, actual_behavior, location_quest, hardware_specs, graphics_mode, driver_os, troubleshooting_tried, pers_id, official_report_submitted, evidence_url, moderation_status, cluster_id. No limit, no pagination, no date filter.
- **Writes:** read-only
...
- **Revalidates:** path /admin; revalidatePublicSurfaces()
- **On failure:** Bad input -> throw "bad input". getCurrentPatchMetadata failure propagates before any write. Update error -> throw error.message; no revalidation runs.
- **Tests:** tests/adminActions.test.ts
- **Quirks:** OVERWRITE, NO HISTORY: the prior fix_status, fix_claimed_at and fix_claimed_patch_version are gone. Setting a claim-bearing status FABRICATES a fix-claim clock timestamped now() and attributed to the current patch, indistinguishable in the table from a real Pearl Abyss claim — the only marker is admin_override=true. Label mismatch: the button says 'Lock' but the select carries the value; submitting without changing the select re-locks at the current status and rewrites the timestamps.
...
- **Quirks:** Self-reversing by design: the next successful scraper sync reclaims control by flipping is_current back (comment at actions.ts:259-261), so this override is silently temporary and no UI says when it will expire. The manual row deliberately carries null published_at and null summary — any surface that assumes those are non-null on the current patch will render blank under an override. The disclosure's summary badge is the only place the operator sees 'Synced X' vs 'Fallback X' (page.tsx:284-289); losing it removes the only cue that the scraper is broken.
...
- **Does:** Reads the whole evidence base, builds a deterministic markdown dossier, optionally rewrites it with AI, stores the run, and redirects to it.
- **Backing:** src/app/admin/actions.ts:281
- **Inputs:** FormData: use_ai (checkbox, "on"). Reads: all approved bug_reports (category, platform, cluster_id, evidence_url, repro_steps, issue_title); all issue_clusters (id, title, fix_status, confidence); source_signals where public_status='public' ordered by observed_at desc; approved_excerpts joined to bug_reports, newest first, limit 1000; a head-only count of pending reports. Caps: 15 repro notes, 30 evidence URLs.
...
- **On failure:** Each read failure throws a labelled `<label> read failed: <msg>` via throwReadError (actions.ts:83). Insert failure throws error.message. If AI is requested but features().ai is false or draftDossierWithAi returns null, it silently falls back to the deterministic text with provider='deterministic' — no message tells the operator the AI step was skipped. On success it redirect()s to /admin/compile?run=<id>.
- **Tests:** tests/dossier.test.ts; tests/adminActions.test.ts
- **Quirks:** APPEND-ONLY: every compile inserts a new dossier_runs row and nothing ever deletes one — there is no admin surface listing or pruning past runs, only the ?run= deep link the redirect hands you. approved_excerpts is read with limit 1000 and de-duplicated by report_id in application code (actions.ts:87-100), so past 1000 excerpts the verified-report counts silently under-report. It reads verified reports through a Supabase relational join (bug_reports(...)) that can come back as an object or an array depending on row shape — relatedReport() at actions.ts:79 exists solely to absorb that.
...
- **Tests:** tests/adminGuard.test.ts; tests/session.test.ts; tests/automationPreview.test.ts
...
- **Guard:** requireAdmin() at src/app/admin/actions.ts:112 and assertProductionWriteAllowed() at :113. NEITHER IS PINNED — tests/adminActions.test.ts:24 replaces @/lib/adminGuard with a stub and no test in the repo asserts requireAdmin was called for this or any other action.
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:168 + assertProductionWriteAllowed() :169 — NOT PINNED (adminGuard stubbed; no preview-mode test for this action).
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:235 + assertProductionWriteAllowed() :236 — NOT PINNED.
- **Revalidates:** revalidatePath("/admin") — NOT asserted; revalidatePublicSurfaces() — NOT asserted
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:198 + assertProductionWriteAllowed() :199 — NOT PINNED.
...
- **Tests:** force_public writes the escape hatch and immediately refreshes effective visibility; force_hidden removes a quiet cluster from public reads before the deeper refresh; auto clears the override back to engine control; revalidates the applied override when the immediate refresh fails
...
- **Guard:** Input validation only; auth guard above it is stubbed.
- **Revalidates:** —
...
- **Guard:** Inherits the caller's guards (stubbed in test).
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:447 (stubbed, not pinned) + assertProductionWriteAllowed() :448 — THE PREVIEW GUARD IS GENUINELY PINNED (tests/adminActions.test.ts:442 sets VERCEL_ENV=preview and @/lib/previewGuard is NOT mocked in this file).
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:458 (stubbed) + assertProductionWriteAllowed() :459 — NOT pinned for this action (no preview test).
...
- **Guard:** NONE OF ITS OWN — src/app/admin/actions.ts:697 has no requireAdmin and no preview guard; it inherits them from recordScannerDecision at :471-472. No test asserts this.
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:471 (stubbed) + assertProductionWriteAllowed() :472 — NOT pinned.
...
- **Quirks:** Pins the CONTRACT. The negative assertions are the real content — this control must not leak into the visibility system. Coupled to the Undo control: every decision recorded here is only reversible because tests/queriesAdminCompatibility.test.ts:189 pins that ALL active rules are loaded (no limit), so nothing falls past a page cap and becomes permanent.
...
- **Does:** Promotes an auto-rejected candidate back into the signal pipeline AND records a durable allow rule, in that order.
...
- **Writes:** source_signals via rescueCandidateSignal — PINNED only as "called once"; RPC record_scanner_decision with p_decision="relevant" — PINNED (tests/adminActions.test.ts:657-660)
- **Guard:** requireAdmin() + assertProductionWriteAllowed() at src/app/admin/actions.ts:471-472 — NOT pinned.
- **Revalidates:** not asserted
...
- **Quirks:** Pins the CONTRACT including ORDERING — tests/adminActions.test.ts:661 asserts rescueCandidateSignal's invocation order is strictly before the RPC's. A redesign that parallelizes these two writes for speed breaks a deliberate atomicity property. The reason text is baked into a hidden input in the component, so the operator never sees or edits what gets stored as their justification.
...
- **Guard:** requireAdmin() + assertProductionWriteAllowed() src/app/admin/actions.ts:471-472 — NOT pinned. The signal-specific input guard at :488 IS pinned.
...
- **Guard:** Content-based refusal at src/app/admin/actions.ts:512; the auth guard above it is stubbed.
- **Revalidates:** —
...
- **Writes:** RPC undo_scanner_decision(p_decision_id) — PINNED exactly (tests/adminActions.test.ts:820). What it flips (scanner_feedback_rules.revoked_at, scanner_decisions, patch_observations.is_public) is NOT verified by any test.
- **Guard:** requireAdmin() src/app/admin/actions.ts:709 (stubbed) + assertProductionWriteAllowed() :710 — NOT pinned.
...
- **Tests:** revokes the learning rule without touching cluster visibility; recomputes the affected signal cluster after undo
...
- **Guard:** requireAdmin() src/app/admin/actions.ts:264 (stubbed) + assertProductionWriteAllowed() :265 — THE PREVIEW GUARD IS GENUINELY PINNED (tests/adminActions.test.ts:869).
...
- **Tests:** rejects a malformed patch version with no writes; writes the manual current patch through one atomic RPC and refreshes public surfaces; blocks the override in Vercel preview; surfaces an atomic write failure instead of claiming success
...
- **Guard:** isAdmin() -> 401 (PINNED, tests/adminScanRoute.test.ts:98) and isVercelPreview() -> 403 {error:"preview_writes_disabled"} (PINNED, :105). This is the ONLY admin surface whose auth guard is genuinely asserted by a test.
...
#### `api-admin-scan-status-get` — (automatic) scan progress polling (GET /api/admin/scan/status?id=)

- **Kind:** automatic · **Destructive:** none
- **Reach:** Fired every 2500ms by ScanControls while a run is active (src/components/ScanControls.tsx:60)
- **Does:** Echoes a run row's status/progress and, as a side effect, revalidates public surfaces when it observes a recently finished manual run.
...
- **Writes:** read-only, except cache revalidation; sweepStaleRuns() is invoked on the 404 path — PINNED as called once (tests/adminScanRoute.test.ts:210); what it writes is mocked and unverified
...
- **On failure:** PINNED: unknown id -> 404 (and sweeps stale runs first); read error -> 500 (tests/adminScanRoute.test.ts:268).
...
- **Quirks:** Pins the CONTRACT. A GET that mutates cache state is surprising and load-bearing: this is the fallback that publishes a completed scan if the after() callback never runs. The freshness window (finished within N seconds) is pinned only at the 30s example — the exact boundary is not tested. A redesign that replaces polling with a websocket or removes the status endpoint silently deletes this second revalidation path.
...
- **Guard:** The route itself IS the guard. NOT PINNED: no test exercises POST /api/admin/login at all — not the 400 invalid_json branch, not the 401 invalid_credentials branch, not the deliberate 750ms timing delay at src/app/api/admin/login/route.ts:14, not the cookie attributes.
- **Revalidates:** —
...
- **Writes:** read-only
- **Guard:** requireAdmin() at src/app/admin/page.tsx:25 gates the page — NOT PINNED by any test.
...
#### `get-automation-admin-data-on-page-load` — (automatic) /scanner admin data read (candidates, rules, observations)

- **Kind:** automatic · **Destructive:** none
- **Reach:** Runs on every authenticated /scanner render; feeds the teaching desk, rules list, and Wire/Asks moderation
- **Does:** Loads rejected candidates, active feedback rules, observations, and their decision ids — with rolling-migration fallbacks and hard failures for anything unrelated.
- **Backing:** tests/queriesAdminCompatibility.test.ts:108; reader src/lib/queries.ts (getAutomationAdminData)
- **Inputs:** None (server read).
- **Writes:** read-only
- **Guard:** Page-level requireAdmin/isAdmin on /scanner — NOT PINNED here.
- **Revalidates:** none
- **On failure:** PINNED: unrelated read failures throw with named messages — "rejected candidates read failed:", "scanner feedback rules read failed:", "observations read failed:". PINNED: only a MISSING RELATION (PGRST205) degrades to an empty list, and it flips feedbackLearningAvailable to false rather than pretending.
- **Tests:** retries a legacy rejected-candidate projection and normalizes feedback fields; surfaces unrelated rejected-candidate read failures; treats only a missing feedback-rules relation as the legacy empty state; enables scanner-learning controls when the feedback schema is available; loads every active feedback rule so each enforced lesson keeps an Undo path; filters decided and rescued candidates before the thirty-row window; surfaces unrelated feedback-rule read failures; surfaces unrelated observation read failures instead of an empty desk; treats only a missing patch_observations relation as an empty desk; scopes the decision lookup to the listed observations so no Undo falls past the cap
- **Quirks:** Pins the CONTRACT, and two of these are pure REDESIGN TRAPS. (1) tests/queriesAdminCompatibility.test.ts:189 asserts the feedback-rules query has NO limit — every enforced rule must be listed so every rule keeps an Undo; adding pagination to that list makes rules permanently un-undoable. (2) :203 asserts decided/rescued candidates are filtered BEFORE the limit:30 window, verified by comparing operation indexes — reordering the query builder silently fills the desk with already-handled rows. (3) :258 asserts the decision lookup is scoped by `in:observation_id:` to exactly the observations being rendered.
...
- **Tests:** freezes teaching-desk relative times at the server-captured instant; keeps every retained lead reachable and gives each one an explicit teaching action; gives each public context item one explicit action and hidden items an undo; hides scanner-learning actions until the feedback schema is available
...
- **Quirks:** Pins CONTRACT and COPY together, and it is the ONLY test in the repo that proves the admin area is actually protected — every unit test stubs requireAdmin. If a redesign moves the entry point off the footer or renames the "Admin password" label, this test is the first thing to break and the last line of auth defence. The test uses keyboard activation deliberately (comment at :1027-1029: mobile emulation skews click coordinates at page bottom) — a redesign that moves the entry point out of the footer should keep that in mind. Note the sign-out button is a submit inside a form with display:contents, styled as a nav link: it LOOKS like navigation and IS a mutation.
...
**Surface notes.** BLUNT COVERAGE ASSESSMENT — where a redesign can silently break admin behavior.  THE SINGLE BIGGEST HOLE: no unit test anywhere proves an admin server action is authenticated. tests/adminActions.test.ts:24 replaces @/lib/adminGuard with `requireAdmin: vi.fn()` and never asserts it was called. All 13 write actions (moderateReport, setClusterFixStatus, setClusterVisibilityOverride, clearClusterFixStatusOverride, setCurrentPatchOverride, compileDossier, setAutomationPaused, setScannerPolicy, recordScannerDecision, rejectObservationAndTeach, rescueRejectedCandidate, undoScannerDecision, signOutAdmin) would pass the full unit suite with `await requireAdmin()` deleted. Only three surfaces genuinely pin auth: POST /api/admin/scan and GET /api/admin/scan/status (tests/adminScanRoute.test.ts:98, :195, where isAdmin is asserted to produce 401) and the single e2e test at tests/e2e/public-visual.spec.ts:1053-1056 that proves /admin redirects to /admin/login after sign-out. If that one e2e test is deleted or the footer entry point is redesigned away, the admin area has no authentication coverage at all.  ZERO-COVERAGE CONTROLS (nothing, not even a render assertion): - rejectObservationAndTeach (src/app/admin/actions.ts:619) — the "Reject and teach…" action for Wire/Asks items. Its BUTTON is asserted to render (tests/adminScannerView.test.ts:203) but the handler is mocked out; nothing tests its input guards, its record_observation_decision RPC arguments, its "already hidden — undo first" refusal at :653, or its named-migration error at :682. It is the newest write path on the operator desk and the least protected. - compileDossier (src/app/admin/actions.ts:281) — no unit test. tests/dossier.test.ts covers only buildDeterministicDossier (the pure function); the action's Supabase reads, dossier_runs insert, AI-vs-deterministic branch, and the `use_ai` checkbox have no coverage. E2E only asserts the button exists. - GET /api/admin/export (src/app/api/admin/export/route.ts) — the EXPORT CSV nav item (src/components/dispatch/Chrome.tsx:158). Downloads all 22 columns of bug_reports including raw description, hardware_specs, pers_id and evidence_url. No route test, no e2e, not even a link assertion. tests/csv.test.ts covers buildCsv in isolation only. This is the highest-sensitivity control in the admin area and it is completely untested. - POST/DELETE /api/admin/login (src/app/api/admin/login/route.ts) — no unit test. The 400 invalid_json branch, the 401 branch, the deliberate 750ms delay, the cookie flags (httpOnly/secure/sameSite/12h maxAge), and the entire DELETE branch are unexercised. DELETE appears to be dead: sign-out goes through the signOutAdmin server action instead. - ScanControls (src/components/ScanControls.tsx) — the two scan buttons ("Run capped scan now" and the dry-run button), the 2500ms polling loop, the 4-failure tolerance, the router.refresh() on completion, and the 401 "Your session expired" recovery. tests/adminScannerView.test.ts:15 mocks the component to null; the e2e admin-scanner test never asserts a scan button. The API side is well covered, the operator's only way to reach it is not. - signOutAdmin (src/app/admin/actions.ts:39) — no unit test; e2e-only. - CopyDossierButton / DossierOutput, LoginForm's own client logic, and /admin/source-monitor's redirect — no coverage. - The four /admin stat-band cells (Needs you / Auto-sorted / Flagged reports / Filtered as spam) and the `needsYou` formula at src/app/admin/page.tsx:52 — no unit test; only inside a screenshot.  DEAD OR UNREACHABLE, YET TESTED (a redesign will "delete dead code" and break the suite): - setAutomationPaused — two tests (tests/adminActions.test.ts:441) but NO component imports it. The live pause control is the cadence select feeding setScannerPolicy. - rescueRejectedCandidate — four tests, but its only caller is src/components/scanner/RejectedArchive.tsx, and nothing in src/ imports RejectedArchive. Unreachable button, fully tested. - runRedditMonitor — one test pinning that it always throws; no caller. It is a policy tripwire, not a feature. - Every action's revalidatePath("/admin/source-monitor") is pinned by tests (e.g. tests/adminActions.test.ts:469, :503, :549) but that route is now a bare redirect() to /scanner (src/app/admin/source-monitor/page.tsx:5). - "acknowledged" is excluded from the Lock menu as a dead state (src/app/admin/page.tsx:21-22) yet still accepted by the server action, since FIX_STATUSES contains it.  ALSO NOTE: revalidatePublicSurfaces (src/lib/revalidate.ts:19) swallows every error in a bare catch. No test asserts a failed revalidation is visible anywhere, so cache-refresh regressions will be silent by construction.  DOC CONSTRAINTS ON REORGANIZING THE ADMIN AREA (quoted): - docs/OPERATIONS.md:90-97 fixes the workflow order — "Open `/admin/login` or the footer `Admin` link", "Use the admin report queue to approve, reject, or excerpt direct reports", "Use `/scanner` to inspect run health, pause/resume scheduled scans, run a no-publish test, or run an authorized capped scan", "Use exception controls only for real lifecycle or visibility problems. `Auto` returns a cluster to the engine-owned baseline", "Use `/admin/compile` when a maintainer needs an evidence dossier". Any reorganization that merges /scanner's operator controls into /admin, or drops the "Auto" reset, contradicts the published guide. - docs/OPERATIONS.md:99 — "Publishing a lead or excerpt is a moderation decision, not an automatic consequence of discovery." No redesign may auto-publish from the queue. - docs/OPERATIONS.md:118 — "Do not raise a provider cap, add a provider, enable Reddit API, or change publishing thresholds as an emergency workaround." The $2 / 1000-credit ceilings are policy, pinned in two places. - docs/OPERATIONS.md:74 — high-value provider usage is "capped at `$2` per UTC month." - docs/ARCHITECTURE.md:12 — "/scanner: Public source-radar health and mapped questions; operator controls are shown only after admin authentication." The public/operator split on one route is architectural, not incidental; the paired e2e tests (public-visual.spec.ts:871 and :930) enforce it by asserting operator strings are absent from the public HTML. - docs/ARCHITECTURE.md:46 — "raw reports, rejected candidates, network hashes, individual confirmation rows, credentials, and private moderation details remain outside the public read model." - docs/NEXT-STEPS.md:37 — "Consider a formal contributor/admin identity model only if shared-password operations become a real bottleneck." Single shared password is a deliberate current state, so per-user audit trails are out of scope for now. - docs/NEXT-STEPS.md:35 — "Add operational alerting only when the current run ledger and scanner page are insufficient." The scanner page IS the alerting surface; removing the run ledger removes the monitoring story. - docs/OPERATIONS.md:96 also notes the redesign-relevant fact that exception controls are break-glass only — the /admin disclosures are collapsed by design, and the e2e test at public-visual.spec.ts:987-1003 pins that the override browser stays empty until searched.
