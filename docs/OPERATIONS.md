...
- `AUTOMATION_BUDGET_USD_MONTHLY` is an additional deployment ceiling: the lower of it and the saved budget applies to scans, rescues, and paid preview checks. Set it to `1` to permit the full private control range, or `0` to stop paid AI requests. If omitted, it defaults to `$0.50`.
