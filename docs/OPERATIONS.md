...
High-value automation responses must report their usage cost. A verified cost within both ceilings is recorded against the current UTC month. Missing cost data or a cost above the request/month ceiling stops later LLM calls in that run and opens the automation circuit for later runs in the same UTC month; Tavily and deterministic extraction continue. If monthly run history cannot be read, the automation run fails closed before provider work. The no-write source-preview route is always deterministic-only, so it cannot bypass the circuit.
