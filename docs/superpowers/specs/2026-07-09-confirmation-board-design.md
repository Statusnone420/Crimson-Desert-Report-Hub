...
**Date:** 2026-07-09
**Status:** Approved by owner and implemented on the working branch. Recovery-audit amendment below is authoritative. The confirmation, visibility-guard, lock-order, visibility-baseline, visibility-revision, and visibility-recompute migrations were explicitly authorized and applied successfully under their final remote versions; release verification and deployment remain separate. Supersedes `2026-07-09-autonomy-destination-roadmap.md` where they conflict (see §Supersession).
...
- `20260709210229_visibility_override_guards.sql` adds a service-role visibility RPC that makes `Force public` / `Force hidden` immediate and atomic; database triggers preserve forced state across concurrent scanner writes. `20260709234750_visibility_override_baseline.sql` preserves the starting automatic baseline. `20260710001212_visibility_refresh_revision.sql` keeps that baseline current while forced, makes approved-report promotion transactional, and applies each recomputation atomically behind a revision check. `20260710005327_visibility_restore_recompute.sql` corrects any legacy restore baseline copied from forced effective visibility. `Auto` restores the live engine-owned baseline and immediately recomputes current reports and source rows before revalidating public surfaces.
...
- `20260709210222_issue_confirmations.sql`, `20260709210229_visibility_override_guards.sql`, `20260709212531_visibility_write_lock_order.sql`, `20260709234750_visibility_override_baseline.sql`, `20260710001212_visibility_refresh_revision.sql`, and `20260710005327_visibility_restore_recompute.sql` were explicitly authorized and applied successfully on 2026-07-09. That completed authorization is specific to those migrations; future database changes still require current-message owner authorization. Deployment remains separate.
...
- **High-value LLM lane:** scanner extraction and official fix-claim mapping use only `deepseek/deepseek-v4-flash`. Software caps the lane at $2 per UTC month, applies provider-routing ceilings of $0.10 per million prompt tokens and $0.20 per million completion tokens, reserves worst-case request cost before sending, and opens a same-month circuit when usage cost cannot be verified or exceeds a ceiling. *(Circuit clause superseded 2026-07-10, PR #25: a lone unverified cost now debits the worst-case ceiling and keeps the lane on; the circuit opens for 3+ unverified runs inside a rolling 24 hours, or for the rest of the UTC month on any out-of-bounds cost. See docs/OPERATIONS.md.)*
