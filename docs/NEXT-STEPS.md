...
6. With the required access, inspect `/operator` and `/scanner`. Overview shows the last completed run, next eligible attempt, provider health, and screening counters; Scanner holds diagnostics and collection records. Do not infer private health from a successful public page load.
