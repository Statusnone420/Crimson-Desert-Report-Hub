...
test("newspaper editorial routes retain the brand and separate coverage from scanner snippets", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: "Charting the Unknown", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "October 15 at 6 pm Eastern" })).toBeVisible();
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute("href", "https://crimsonreporthub.com/topics/charting-the-unknown");
  await expect(page.getByRole("link", { name: "Watch on YouTube ↗" })).toHaveCount(coverageVisible ? 1 : 0);
  mkdirSync("output/playwright", { recursive: true });
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ animations: "disabled", path: `output/playwright/editorial-topic-${testInfo.project.name}.png`, fullPage: true });
