...
const STATE_ACTIONS: { name: keyof Actions; form: Record<string, string> }[] = [
  ...WRITE_ACTIONS.map(({ form, name }) => ({ name: `${String(name)}State` as keyof Actions, form })),
];
...
describe("video review write actions require authentication", () => {
  ...
  for (const { name, form } of STATE_ACTIONS) {
    it(`${String(name)} performs no database work for an unauthenticated caller`, async () => {
      const actions = await import("@/app/admin/videos/actions");
      const action = actions[name] as (state: unknown, formData: FormData) => Promise<unknown>;
      await expect(action(initialVideoActionState, buildFormData(form))).rejects.toThrow(AUTH_SENTINEL);
      expect(mocks.requireAdmin).toHaveBeenCalled();
      expect(mocks.from).not.toHaveBeenCalled();
      expect(mocks.revalidatePath).not.toHaveBeenCalled();
    });
  ...
  it("covers every exported video-inbox action", async () => {
    ...
    expect(new Set(Object.keys(actions).filter((key) => typeof (actions as Record<string, unknown>)[key] === "function"))).toEqual(
      new Set([...WRITE_ACTIONS, ...STATE_ACTIONS].map(({ name }) => String(name))),
    );
  });
});
