...
- The page is a centered paper up to 1440px wide. A double top rule, dateline, theme control, ruled navigation, and compact trust footer frame every public route. The front page keeps the full serif masthead; interior pages use a smaller linked nameplate to bring their content into view sooner. The footer provides Watch and original-report RSS/Atom links.
...
- The Patch Desk separates official fix claims, the player record, and claim verdicts. The Issue Board separates published issues from watched scanner leads and keeps report counts, check-ins, public excerpts, source leads, and fix-poll results distinct. Confirmed current claim pairings show the exact official wording with a link to that line in the patch record. Unavailable or unmatched claim context stays explicit. Boards without a watchlist use a centered single reading column.
...
- The report flow is a compact editorial form with category choices, expandable optional details, an explicit review step, and a separate send action. Entry from a visible issue retains its title as context and selects its category; the player still writes their own summary and description.
- Layouts collapse into single-column mobile compositions at their defined breakpoints. The front-page lead places its headline, description, and reading link before the artwork at 900px and below. On phones, the report shortcut moves to the dateline beside the catch-up and theme controls. At 650px and below, the masthead wraps and editorial grids stack; the secondary original report keeps a small image beside its heading and places its summary below. The front page is tested at 320px wide without document-level horizontal scrolling.
...
- Light mode is the primary newspaper edition and the server and first-visit default. The theme control switches the document and browser theme color between dark and light palettes and stores the choice locally. Installed-app launch colors use the light paper. The report CAPTCHA follows the selected theme and replaces its challenge and response together when the theme changes.
...
- Public check-ins use outlined choices with separate numeric compartments. The selected surface moves briefly between choices. Choosing a result reveals the platform picker, then a clear pending, recorded, or retry state. Cancel and Escape restore focus; pending submissions disable the entire picker. Check-ins and report counts remain separate in labeled platform tables, and counts refresh from server data.
- Shared interaction styles explicitly outrank the newspaper's element reset. Reading links stay visibly identifiable, filters show selected and disabled states, and form fields use consistent borders. The lead story has the strongest reading action; secondary source links remain light.
- Standalone destinations use `ReadingLink`: `action` for a page destination, `quiet` for supporting navigation, and `source` for a ruled resource row with optional provenance. Directional SVGs are decorative; the visible text supplies the accessible name. Feeds use native anchors, and inline citations retain their prose treatment.
- `SectionNavigation` supplies the Observatory's sticky page map and the articles' contents rails. A brief Motion marker follows the current section without rewriting the URL. Native fragment links preserve browser history and keyboard navigation; their destinations accept focus and remain clear of the sticky map. Mobile and desktop article contents share one section list.
