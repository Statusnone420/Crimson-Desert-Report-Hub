-- Follow-up to the atomic observation persistence function. External search
-- providers may return a non-null but malformed published date; that optional
-- field must not roll back valid observations in the same batch.
    ...
    if existing_id is not null then
      ...
      if nullif(pg_catalog.btrim(observation->>'source_published_at'), '') is not null then
        ...
        exception when others then
          -- published_date is external and optional; discard only the bad
          -- timestamp while preserving the rest of this observation batch.
          parsed_source_published_at := null;
        end;
      end if;
