  ...
  return <div className="cu-page" onClick={jumpToChapter}>
    <section className="cu-intro" aria-labelledby="cu-title">
      <div className="cu-intro-copy">
        ...
        <div className="cu-start"><CatchUpMenu label="Choose where to begin"/><ReadingLink variant="quiet" direction="down" href={catchUpHash(selection, "cu-brief")}>Read the brief</ReadingLink></div>
      </div>
    ...
    <section className="cu-brief" id="cu-brief" tabIndex={-1} aria-labelledby="cu-brief-title">
      ...
      {milestones.length ? <ol className="cu-brief-grid">{briefItems.map((item, index) => <li key={item.id}><span className="cu-brief-number">{String(index + 1).padStart(2, "0")}</span><h3><a href={catchUpHash(selection, item.id)}>{item.title}</a></h3><p>{item.summary}</p><a className="cu-text-link" href={catchUpHash(selection, item.id)}>In the journey <DirectionIcon direction="down" /></a></li>)}</ol> : <div className="cu-empty"><p>No newer entries in this edition. Choose an earlier date or view the highlights.</p><CatchUpMenu label="Change starting point"/></div>}
    </section>
    <div className="cu-journey" ref={journey}>
      <div className="cu-journey-filter"><p>{catchUpSelectionLabel(selection)} · {milestones.length} entries</p>{selection.kind !== "all" && <ReadingLink variant="quiet" href="#history=all" onClick={showAllHistory}>Show all history</ReadingLink>}</div>
      {milestones.length > 0 && <>
        <aside className="cu-rail" aria-label="Journey chapters"><p className="cu-rail-title">The journey</p><p className="cu-rail-selection">{catchUpSelectionLabel(selection)}</p>{selection.kind !== "all" && <ReadingLink variant="quiet" className="cu-show-all" href="#history=all" onClick={showAllHistory}>Show all history</ReadingLink>}<div className="cu-rail-links" ref={railLinks}><div className="cu-rail-track"><div className="cu-progress" ref={progress} aria-hidden="true"/>{milestones.map((item, index) => <a key={item.id} href={catchUpHash(selection, item.id)} aria-current={active === item.id ? "step" : undefined}><span>{String(index + 1).padStart(2, "0")}</span><span>{catchUpDate(item.publishedAt)}<small>{item.patch ? `Patch ${item.patch}` : "What’s ahead"}</small></span></a>)}</div></div><p className="cu-rail-footnote">Oldest to newest.<br/>Official sources at every stop.</p></aside>
        <div className="cu-chapters">{milestones.map((item, index) => <article key={item.id} id={item.id} tabIndex={-1} className={`cu-milestone cu-milestone--${item.kind}`}>
          ...
          <h2>{item.title}</h2>
          {item.image && <figure className={`cu-chapter-image${item.image.src.endsWith("coast.jpg") ? " cu-coast" : ""}`}><Image {...item.image} alt={item.image.alt} sizes="(max-width: 750px) 100vw, 75vw" loading={index === 0 || item.id === parseCatchUpChapter(hash) ? "eager" : "lazy"}/><figcaption>Image: Pearl Abyss</figcaption></figure>}
