...
export function CatchUpExperience() {
  ...
  const briefs = milestones.filter((item) => item.kind !== "hotfix").slice(0, 3);
  const briefItems = briefs.length ? briefs : milestones.slice(0, 3);
  ...
  return <div className="cu-page" onClick={jumpToChapter}>
    ...
    <div className="cu-edition"><span>{catchUpSelectionLabel(selection)}</span><span>Edition: August 25 – September 4, 2026</span><div className="cu-share"><button type="button" onClick={share}>Copy link <span aria-hidden="true">↗</span></button><span role="status">{shareNotice}</span></div></div>
    {selection.kind === "since" && Date.parse(selection.value) < Date.parse(CATCH_UP_COVERAGE_START) && <p className="cu-coverage">This edition begins August 25. Earlier changes are not covered here.</p>}
    <section className="cu-brief" id="cu-brief" tabIndex={-1} aria-labelledby="cu-brief-title">
      ...
      {milestones.length ? <ol className="cu-brief-grid">{briefItems.map((item, index) => <li key={item.id}><span className="cu-brief-number">0{index + 1}</span><h3><a href={`#${item.id}`}>{item.title}</a></h3><p>{item.summary}</p><a className="cu-text-link" href={`#${item.id}`}>In the journey <span aria-hidden="true">↘</span></a></li>)}</ol> : <div className="cu-empty"><p>No newer entries in this edition. Choose an earlier date or view the highlights.</p><CatchUpMenu label="Change starting point"/></div>}
    </section>
    <div className="cu-journey" ref={journey}>
      {milestones.length > 0 && <>
        <aside className="cu-rail" aria-label="Journey chapters"><p className="cu-rail-title">The journey</p><div className="cu-rail-links"><motion.div className="cu-progress" style={{ scaleY: reduced ? 1 : scrollYProgress }} aria-hidden="true"/>{milestones.map((item, index) => <a key={item.id} href={`#${item.id}`} aria-current={active === item.id ? "step" : undefined}><span>0{index + 1}</span><span>{catchUpDate(item.publishedAt)}<small>{item.patch ? `Patch ${item.patch}` : "What’s ahead"}</small></span></a>)}</div><p className="cu-rail-footnote">Oldest to newest.<br/>Official sources at every stop.</p></aside>
        <div className="cu-chapters">{milestones.map((item, index) => <article key={item.id} id={item.id} tabIndex={-1} className={`cu-milestone cu-milestone--${item.kind}`}>
          <div className="cu-chapter-meta"><span className="cu-chapter-number">0{index + 1}</span><time dateTime={item.publishedAt}>{catchUpDate(item.publishedAt, true)}</time><span>{item.patch ? `Patch ${item.patch}` : "On the horizon"}</span></div>
          <h2>{item.title}</h2>
