    ...
    <div id={id} ref={popover} popover="auto" role="dialog" aria-labelledby={`${id}-title`} className="catch-up-popover" onBeforeToggle={(event) => synchronizeOpen(event.newState === "open")}>
      <div className="catch-up-menu">
        ...
        {ready && (preferences.caughtUpThrough || previousVisit) && <div className="catch-up-saved">
          {preferences.caughtUpThrough && <button type="button" onClick={() => navigate({ kind: "since", value: preferences.caughtUpThrough!, remembered: true })}>Where I left off <span>{catchUpLocalDate(preferences.caughtUpThrough)} →</span></button>}
          {previousVisit && <button type="button" onClick={() => navigate({ kind: "since", value: previousVisit, remembered: true })}>Since my last visit <span>{catchUpLocalDate(previousVisit)} →</span></button>}
        </div>}
