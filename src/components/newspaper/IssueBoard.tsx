...
function subscribeToIssueHash(listener: () => void) {
...
export function IssueBoard({ published, watchlist, monitoredCount, emptyPatchVersion }: { published: IssueBoardEntry[]; watchlist: IssueBoardEntry[]; monitoredCount: number; emptyPatchVersion: string }) {
  const issueHash = useSyncExternalStore(subscribeToIssueHash, issueHashSnapshot, serverIssueHash);
  const [selection, setSelection] = useState<{ hash: string; view: BoardView } | null>(null);
  const linkedIssue = [...published, ...watchlist].find((issue) => issueHash === `#issue-${issue.id}`);
  const view = selection?.hash === issueHash ? selection.view
    : watchlist.some((issue) => issue.id === linkedIssue?.id) ? "watchlist" : "published";
  function setView(nextView: BoardView) { setSelection({ hash: issueHash, view: nextView }); }
  useEffect(() => {
    if (linkedIssue) document.getElementById(`issue-${linkedIssue.id}`)?.scrollIntoView({ block: "start" });
  }, [linkedIssue, view]);
  const viewId = useId();
...
function PublishedIssue({ issue }: { issue: IssueBoardEntry }) { const art = artFor(issue.category); const pollTotal = (issue.poll?.fixedCount ?? 0) + (issue.poll?.stillCount ?? 0); const fixedPercent = pollTotal > 0 ? Math.round(((issue.poll?.fixedCount ?? 0) / pollTotal) * 100) : 0; return <article id={`issue-${issue.id}`} className={`report-dispatch issue-category-${issue.category}`} aria-labelledby={`title-${issue.id}`}><div className="dispatch-topline"><span className="dispatch-category"><CategorySymbol category={issue.category} />{issue.categoryLabel}</span><span className={`dispatch-status dispatch-status--${issue.tone}`}>{issue.status}</span></div><div className="dispatch-heading"><div><h2 id={`title-${issue.id}`}>{issue.title}</h2><p className="dispatch-byline">{issue.checkinsAvailable === false ? "Current check-ins unavailable" : `${plural(issue.confirmationCount, "check-in")} · Patch ${issue.storageScope}`}{issue.directReportCount > 0 ? ` · ${plural(issue.directReportCount, "earlier report")}` : ""}</p></div><figure><Image src={art.image} alt={art.alt} width={1920} height={1080} sizes="(max-width: 750px) 100vw, 160px" style={{ objectPosition: art.position }} /><figcaption>Image: Pearl Abyss</figcaption></figure></div>{issue.description ? <p className="dispatch-description">{issue.description}</p> : null}<dl className="dispatch-facts">{issue.platformCounts.length === 0 ? <div><dt>Platform</dt><dd>{issue.reportPlatforms.length > 0 ? issue.reportPlatforms.join(", ") : "No platform responses yet"}</dd></div> : null}<div><dt>Source leads</dt><dd>{plural(issue.sourceLeadCount, "public lead")}</dd></div><div><dt>Community record</dt><dd>{issue.sentence}</dd></div></dl>{issue.platformCounts.length > 0 ? <table className="dispatch-platform-record"><caption>Current-patch check-ins and earlier reports by platform</caption><thead><tr><th scope="col">Platform</th><th scope="col">Earlier reports</th><th scope="col">Check-ins</th></tr></thead><tbody>{issue.platformCounts.map((row) => <tr key={row.label}><th scope="row">{row.label}</th><td>{row.reports}</td><td>{issue.checkinsAvailable === false ? "Unavailable" : <>{row.confirmations}{row.experiences ? <small className="platform-experiences">{row.experiences}</small> : null}</>}</td></tr>)}</tbody></table> : null}{issue.hasCurrentClaim || issue.poll ? <OfficialIssueClaims issue={issue} /> : null}{issue.poll ? <div className="verdict-rail"><p>Check-ins after the official claim</p>{pollTotal > 0 ? <><div className="verdict-bar" role="presentation"><div className="verdict-bar__fixed" style={{ width: `${fixedPercent}%` }} /><div className="verdict-bar__still" style={{ width: `${100 - fixedPercent}%` }} /></div><div className="verdict-labels"><span className="verdict-labels__fixed"><strong>{issue.poll.fixedCount}</strong><span>fixed for me</span></span><span className="verdict-labels__still"><strong>{issue.poll.stillCount}</strong><span>still happening</span></span></div></> : <p className="verdict-quiet">No responses after this claim yet</p>}</div> : null}{issue.excerpts.length > 0 ? <details className="dispatch-excerpt"><summary>Read earlier public reports</summary>{issue.excerpts.map((excerpt, index) => <blockquote key={`${issue.id}-${index}`}>“{excerpt.text}” <cite>— {excerpt.platform} player</cite></blockquote>)}<p>One report describes one player’s experience.</p></details> : null}{issue.sourceLeads.length > 0 ? <details className="dispatch-excerpt"><summary>Read public source leads</summary>{issue.sourceLeads.map((lead) => <p key={lead.id}><a href={lead.url} target="_blank" rel="noreferrer noopener">{lead.source}</a> · {lead.summary}</p>)}<p>Links are leads, never player evidence.</p></details> : null}<IssueActions issue={issue} /></article>; }
function IssueActions({ issue }: { issue: IssueBoardEntry }) {
  const exactClaimMissing = Boolean((issue.hasCurrentClaim || issue.poll) && (issue.officialClaimsUnavailable || !issue.officialClaims?.length));
  return <div className="dispatch-actions">
    {issue.checkinsAvailable === false ? <p className="checkin-unavailable" role="status">Current-patch check-ins could not be loaded. Try again later.</p>
      : exactClaimMissing ? <p className="checkin-unavailable">The exact fix context is unavailable. Check-ins for this claim will open when its source can be shown.</p>
      : issue.ask ? <ConfirmButtons clusterId={issue.id} storageScope={issue.storageScope} question={issue.ask.question} kinds={issue.ask.kinds} counts={issue.confirmationCounts} /> : null}
    {(issue.earlierCheckinCount ?? 0) > 0 ? <p className="checkin-history">{plural(issue.earlierCheckinCount ?? 0, "earlier check-in")} retained from the previous counting system. Excluded from this patch’s tally.</p> : null}
  </div>;
}
function WatchIndex({ watchlist, monitoredCount, categories, openWatchlist }: { watchlist: IssueBoardEntry[]; monitoredCount: number; categories: [string, string][]; openWatchlist: (category?: string) => void }) { const total = watchlist.length + monitoredCount; return <aside className="watch-index" aria-labelledby="watch-index-title"><div className="watch-index-heading"><div><p className="kicker">Still on the radar</p><h2 id="watch-index-title">The watchlist</h2></div><span>{watchlist.length}<small>public leads</small></span></div><p className="watch-index-intro">Public leads across patches. Add your experience on the current patch.</p><div className="watch-index-categories">{categories.map(([key, label]) => { const count = watchlist.filter((issue) => issue.category === key).length; return <button className={`watch-index-category issue-category-${key}`} key={key} onClick={() => openWatchlist(key)} disabled={count === 0}><CategorySymbol category={key} /><span>{label}</span><strong>{count}</strong><span aria-hidden="true">↗</span></button>; })}</div><p className="watch-index-note">Showing {watchlist.length} of {total} watchlist issue{total === 1 ? "" : "s"}{monitoredCount > 0 ? ` · Monitoring ${monitoredCount} additional watchlist issue${monitoredCount === 1 ? "" : "s"}.` : "."}</p><button className="watch-index-all" onClick={() => openWatchlist()}>Explore the full watchlist →</button><div className="watch-index-tip"><p className="eyebrow">A quick check-in is enough.</p><p>Choose what you are seeing and where you play. New topics come from public-source discovery and maintainer curation.</p></div></aside>; }
function Watchlist({ visible, categories }: { visible: IssueBoardEntry[]; categories: [string, string][] }) { return <><p className="watchlist-intro">Public leads, grouped by category. Scanner sightings are not player reports.</p><div className="board-watchlist">{categories.map(([category, label]) => { const matches = visible.filter((issue) => issue.category === category); if (matches.length === 0) return null; const art = artFor(category); return <section className={`watch-category issue-category-${category}`} key={category} aria-labelledby={`watch-category-${category}`}><div className="watch-category-heading"><CategorySymbol category={category} /><h2 id={`watch-category-${category}`}>{label}</h2><span>{plural(matches.length, "lead")}</span></div><figure><Image src={art.image} alt="" width={1920} height={1080} sizes="(max-width: 750px) 100vw, 33vw" style={{ objectPosition: art.position }} /><figcaption>Image: Pearl Abyss</figcaption></figure>{matches.map((issue) => <article id={`issue-${issue.id}`} className="watch-entry" key={issue.id}><h3>{issue.title}</h3><p className="watch-entry-signal">{issue.candidateSignalCount > 0 ? `The scanner spotted this ${plural(issue.candidateSignalCount, "time")} — a lead, not evidence.` : issue.sentence}</p><details><summary>Why it’s watched</summary><p>{issue.sentence}</p></details><IssueActions issue={issue} /></article>)}</section>; })}</div></>; }
