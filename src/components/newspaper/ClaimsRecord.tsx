...
export type PublicClaim = { key?: string; fixText: string; category: string | null; section: string | null };
...
export function ClaimsRecord({ claims, claimsUnavailable, sourceTotal, officialUrl }: { claims: PublicClaim[]; claimsUnavailable: boolean; sourceTotal: number | null | undefined; officialUrl: string }) {
  ...
  if (claimsUnavailable) return <section id="claims" className="claims-record" aria-labelledby="claims-title"><div className="claims-heading"><div><p className="kicker">Inside the update</p><h2 id="claims-title">The official claims record is unavailable.</h2></div><p>The official register could not be read. This is not a report of zero claimed fixes.</p></div><ReadingLink className="action" href={officialUrl} target="_blank" rel="noreferrer noopener">Read Pearl Abyss’s complete notes</ReadingLink></section>;
