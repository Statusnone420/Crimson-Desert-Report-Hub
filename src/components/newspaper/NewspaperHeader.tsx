...
export function NewspaperHeader({ active, date, home = false }: { active?: string; date: string; home?: boolean }) {
  ...
  return <header><div className="topline"><div>{date}</div><div className="theme"><ThemeToggle/></div></div><Masthead className="masthead"><Link href="/">Crimson Desert <em>Report Hub</em></Link></Masthead><nav aria-label="Main navigation">{[["brief", "/", "News"], ["patches", "/patches", "Patches"], ["issues", "/issues", "Player reports"], ["observatory", "/observatory", "Observatory"]].map(([key, href, label]) => <Link key={key} href={href} aria-current={active === key ? "page" : undefined}>{label}{active === key && <motion.span className="nav-indicator" layoutId="newspaper-nav" transition={{ duration: reducedMotion ? 0 : .28 }}/>}</Link>)}<Link className="file" href="/report" aria-current={active === "report" ? "page" : undefined}>File a report <span aria-hidden="true">→</span></Link></nav></header>;
}
