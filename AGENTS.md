...
- Measure a scanner query before changing it: `npm run scan:bakeoff` runs the real query pack against the live Tavily API on the development key and judges every result with the real pre-screen, taking results in the shape production would: urls canonicalized and judged once per modeled run, lane summaries counting only domains that supplied evidence. Put the before and after numbers in the PR, and compare only reports whose headers carry the same dedupe marker line — the report states its own counting rules, and a baseline without that line counted repeats and is not comparable. Unit tests prove the pack is built correctly; only this shows whether the pack is worth running, and the gap between those two hid a scanner that asked two community domains for three weeks. Never reason about search behaviour from the query string alone — `site:` filters silently fall back to open web, and the news index answers the same query differently minutes apart.
- Run migration SQL against the local stack before proposing it: `npm run db:start`, then `npm run db:reset` to rebuild the schema from every migration in order. The Supabase CLI is a pinned dev dependency, so `npm install` is the only setup step besides a running Docker Engine. Migration SQL that has never been executed is not reviewable — say so in the PR rather than arguing its behaviour from the code it mirrors. The local stack is a container on the developer machine; it is never linked to a hosted project, and `supabase db push` stays out of every workflow.
